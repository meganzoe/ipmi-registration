...
class DemonsAlgorithm(object):
  def __init__(self, optDict = {}):
    ...
    self.sigma_elastic = optDict.get('sigma_elastic', 0.5)
    self.sigma_fluid = optDict.get('sigma_fluid', 1)
    self.use_target_grad = optDict.get('use_target_grad', False)
    self.num_lev = optDict.get('num_lev', 3)
    self.max_it = optDict.get('max_it', 1000)
    self.df_thresh = optDict.get('df_thresh', 0.001)
    self.check_MSD = optDict.get('check_MSD', True)
    self.disp_freq = optDict.get('disp_freq', 500)
    self.disp_spacing = optDict.get('disp_spacing', 2)
    self.scale_update_for_display = optDict.get('scale_update_for_display', 10)
    self.disp_method_df = optDict.get('disp_method_df', 'grid')
    self.disp_method_up = optDict.get('disp_method_up', 'arrows')
  ...
  def run( self ):
    ...
    for lev in xrange(1,self.num_lev+1):
        ...
        if lev == self.num_lev:
            ...
            target = rescale(self.target_full, 1.0/resamp_factor, mode='reflect', \
                preserve_range=True, multichannel=False, order =3,  anti_aliasing=True)
            source = rescale(self.source_full, 1.0/resamp_factor, mode='reflect', \
                preserve_range=True, multichannel=False, order =3,  anti_aliasing=True)
        ...
        for it in range(self.max_it):
            ...
            if it % self.disp_freq == 0:
              ...
              fig2, axs = plt.subplots(nrows = 2, ncols = 2, figsize = (7,7))
              utils.dispImage(self.trans_image, ax = axs[0,0])
              axs[0,0].set_title('Transformed image lv: {} it: {}'.format(lev,it))
              utils.dispImage(target - self.trans_image, ax = axs[0,1])
              axs[0,1].set_title('Target - transformed image lv: {} it: {}'.format(lev,it))

              utils.dispDefField(def_field, ax = axs[1,0], spacing = self.disp_spacing, \
                  disptype = self.disp_method_df)
              axs[1,0].set_title('Deformation field lv: {} it: {}'.format(lev,it))
              update_x_tmp = np.expand_dims(update_x, update_x.ndim)
              update_y_tmp = np.expand_dims(update_y, update_y.ndim)
              up_field_to_display = self.scale_update_for_display * np.concatenate((update_x_tmp, update_y_tmp), axis = -1)
              up_field_to_display += np.dstack((X,Y))
              utils.dispDefField(up_field_to_display, ax = axs[1,1], spacing = self.disp_spacing, \
                disptype = self.disp_method_up)
              axs[1,1].set_title('Update lv: {} it: {}'.format(lev,it))
